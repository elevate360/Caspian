...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		...
		if ( is_singular() ) :
			...
			the_title( '<h2 class="entry-title"><a href="' . esc_url( caspian_get_link_url() ) . '" rel="bookmark">', '</a></h2>' );
		endif;
		?>

		<div class="external-link">
			<?php echo sprintf( '<a href="%1$s">%1$s %2$s</a>', esc_url( caspian_get_link_url() ), caspian_get_svg( array( 'icon' => 'external' ) ) );?>
		</div>
