<?php
/**
 * Caspian functions and definitions
 *
...
/**
 * Caspian only works in WordPress 4.7 or later.
 */
...
if ( ! function_exists( 'caspian_setup' ) ) :
/**
 ...
 */
function caspian_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on Caspian, use a find and replace
	 * to change 'caspian' to the name of your theme in all the template files.
	 */
	...
	register_nav_menus( array(
		'menu-1' => esc_html__( 'Primary', 'caspian' ),
		'menu-2' => esc_html__( 'Social Link', 'caspian' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'caspian_custom_background_args', array(
	 ...
	 */
	add_editor_style( array( caspian_fonts_url(), 'assets/css/editor-style.min.css' ) );
...
add_action( 'after_setup_theme', 'caspian_setup' );
 ...
 */
function caspian_content_width() {
	$content_width = $GLOBALS['content_width'];

	/**
	 * Filter Caspian content width of the theme.
	 *
	 * @since Caspian 1.0
	 *
	 * @param int $content_width Content width in pixels.
	 */
	$GLOBALS['content_width'] = apply_filters( 'caspian_content_width', $content_width );
}
add_action( 'template_redirect', 'caspian_content_width', 0 );
 ...
 */
function caspian_widgets_init() {

	register_sidebar( array(
		'name'          => esc_html__( 'Footer Widgets', 'caspian' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'caspian' ),
...
add_action( 'widgets_init', 'caspian_widgets_init' );

/**
 * Register custom fonts.
 */
function caspian_fonts_url() {
	$fonts_url = '';
	 ...
	 */
	$montserrat = _x( 'on', 'Zilla Slab font: on or off', 'caspian' );
 ...
 */
function caspian_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'caspian-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'wp_resource_hints', 'caspian_resource_hints', 10, 2 );
...
function caspian_scripts() {
	...
	$output = array(
		...
		'collapseMenu' 	=> caspian_get_svg( array( 'icon' => 'collapse' ) ),
		'subNav' 		=> '<span class="screen-reader-text">' . __( 'Sub Navigation', 'caspian' ) . '</span>',
		'imageSrc'		=> __( 'Image Source &rarr;', 'caspian' )
	);
	wp_localize_script( 'caspian-script', 'Caspianl10n', $output );
...
add_action( 'wp_enqueue_scripts', 'caspian_scripts' );

/**
 * Handles JavaScript detection.
 * Adds a `js` class to the root `<html>` element when JavaScript is detected.
 */
function caspian_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
}
add_action( 'wp_head', 'caspian_javascript_detection', 0 );
