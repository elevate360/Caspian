...
/**
 * Wrapper function for the Caspian_Media_Grabber class.  Returns the HTML output for the found media.
 *
 * @since  1.6.0
 * @access public
 * @param  array
 * @return string
 */
function caspian_media_grabber( $args = array() ) {

	$media = new Caspian_Media_Grabber( $args );
...
class Caspian_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'caspian_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'caspian_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed'     ) );
		...
		$this->args    = apply_filters( 'caspian_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'caspian_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'caspian_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'caspian_media_grabber_autoembed_media', $value[0] ) );
	...
	public function filter_dimensions( $html ) {
		...
		list( $width, $height ) = apply_filters(
			'caspian_media_grabber_dimensions',
